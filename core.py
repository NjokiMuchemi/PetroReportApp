...
def extract_maintenance_from_workbook(uploaded_file, zone_name: str) -> pd.DataFrame:
    ...
    for sheet_name in find_maintenance_sheet_names(wb):
        ...
        for row in ws.iter_rows(min_row=1, values_only=True):
            ...
            rows.append({
                ...
                "Issue": issue_text,
                "Date Reported": d,
                "Reported By": clean_text(reported_to),
                "Reported To": clean_text(action_person) or "Vijay",
                "Status": "Open",
    ...
    if df.empty:
        return pd.DataFrame(columns=["Zone", "Station", "Issue Type", "Issue", "Date Reported", "Reported By", "Reported To", "Status", "Comments", "Source Sheet"])
    return df
...
def classify_maintenance_issue(issue_type, issue, comments):
    ...
    # Specific tank separation: generator fuel tank is not the same as PMS/AGO/IK product tank.
    if any(w in text for w in ["GENSET", "GENERATOR"]):
        if "TANK" in text and any(w in text for w in ["LEAK", "LEAKING"]):
            return "Generator & Power Systems", "Critical", "Generator fuel tank leak"
        if any(w in text for w in ["NOT START", "NOT WORK", "FAULT", "BATTERY", "AVR", "ATS", "POWER", "INVERTER"]):
            return "Generator & Power Systems", "Critical", "Generator/power failure"
        return "Generator & Power Systems", "Major", "Generator/power issue"
    ...
    if any(w in text for w in ["CARD READER", "GOLD CARD", "VEEDER", "ATG", "AUTOMATION", "CONTROLLER", "COMMUNICATION", "POS"]):
        return "Automation & Forecourt Systems", "Critical", "Automation/control risk"

    if any(w in text for w in ["PUMP", "NOZZLE", "DISPENS", "METER", "PRESET", "FLOW", "HOSE", "BOARD", "MANUAL", "SUCTION", "COUNTING"]):
        return "Dispensing Equipment", "Critical", "Sales-impact dispensing issue"

    if any(w in text for w in ["FIRE", "EARTHING", "EMERGENCY", "SPILL", "SAFETY"]):
        return "Safety & Compliance", "Critical", "Safety/compliance exposure"

    if any(w in text for w in ["LIGHT", "CANOPY", "ROOF", "WALL", "DRAIN", "TOILET", "PLUMB", "SEWER", "DOOR", "TOTEM", "SIGN", "MASON", "STRUCTURE", "ACCESS"]):
...
def build_maintenance_reports(maintenance_df: pd.DataFrame):
    ...
    if "Reported By" not in df.columns and "Reported To" in df.columns and "Assigned To" in df.columns:
    ...
    register = df[["Issue ID", "Zone", "Station", "Category", "Priority", "Issue Type", "Issue", "Date Reported", "Days Open", "Reported By", "Reported To", "Management Reason", "Comments", "Source Sheet"]].copy()
    register = register.sort_values(["Priority", "Days Open", "Zone", "Station"], ascending=[True, False, True, True])
    ...
    critical = critical.sort_values(["Priority", "Days Open"], ascending=[True, False])

    responsibility = register.groupby("Reported To", as_index=False).agg(
        Open_Issues=("Issue ID", "count"),
        Critical_Issues=("Priority", lambda s: (s == "Critical").sum()),
        Oldest_Days_Open=("Days Open", "max"),
...
def to_maintenance_excel_bytes(maintenance_reports):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        responsibility.to_excel(writer, sheet_name="Responsibility Matrix", index=False)
...
if "maintenance_reports" in st.session_state:
    ...
    mtabs = st.tabs(["Pending Register", "Critical Issues", "Station Summary", "Aging Analysis", "Responsibility Matrix", "Unmapped Maintenance"])
    with mtabs[0]:
